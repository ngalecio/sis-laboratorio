...
<body>
        ...
        <table>
            <thead>
                ...
                <tr>
                    <th colspan="5"
                        ...
                        Ajustes de Inventario Registrados
                    </th>
                    <th colspan="2"
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td style="text-align:right;">$ {{ number_format($ajuste->valor_subtotal, 2, '.', ',') }}</td>
                    <td style="text-align:right;">$ {{ number_format($ajuste->valor_iva, 2, '.', ',') }}</td>
                    <td style="text-align:right;">$ {{ number_format($ajuste->valor_total, 2, '.', ',') }}</td>
                </tr>
                @endforeach
                    <!-- Fila de totales -->
                    <tr>
                        ...
                        <th style="background:#e7e7e7; text-align:right;">$
                            {{ number_format($ajustes->sum('valor_subtotal'), 2, '.', ',') }}
                        </th>
                        <th style="background:#e7e7e7; text-align:right;">$
                            {{ number_format($ajustes->sum('valor_iva'), 2, '.', ',') }}
                        </th>
                        <th style="background:#e7e7e7; text-align:right;">$
                            {{ number_format($ajustes->sum('valor_total'), 2, '.', ',') }}
                        </th>
                    </tr>
                @else
                <tr>
                    <td colspan="15" style="text-align:center;">No hay ajustes registrados.</td>
                </tr>
                @endif
            </tbody>
        </table>
    ...
    <script type="text/php">
        if (isset($pdf)) {
                ...
                $pdf->page_text(30, 565, "Ajustes de Inventario", $font, $size, $color);
            }
        </script>
</body>
