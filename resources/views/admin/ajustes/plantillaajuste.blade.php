...
<body>
    <div class="container">
        ...
        <table>
            ...
            <tr>
                <td colspan="3"><strong>Ajuste N°:</strong> {{ $ajustes->numero_comprobante ?? '-' }}</td>
                <td colspan="3"><strong>Fecha:</strong> {{ $ajustes->fecha ? \Carbon\Carbon::parse($ajustes->fecha)->format('d/m/Y') : '-' }}</td>
            </tr>
            <tr>
                <td colspan="3"><strong>Proveedor:</strong> {{ $ajustes->cliente->nombres ?? '-' }} {{ $ajustes->cliente->apellidos ?? '' }}</td>
                <td colspan="3"><strong>Cédula/DNI:</strong> {{ $ajustes->cliente->cedula ?? '-' }}</td>
            </tr>
            <tr>
                <td colspan="3"><strong>Dirección:</strong> {{ $ajustes->cliente->direccion ?? '-' }}</td>
                <td colspan="3"><strong>Teléfono:</strong> {{ $ajustes->cliente->telefono ?? '-' }}</td>
            </tr>
        </table>
        <!-- DETALLE DE AJUSTE -->
        <table>
            ...
            <tbody>
                @php $i = 1; @endphp
                @foreach($ajustes->detalles as $detalle)
                <tr>
                    <td>{{ $i++ }}</td>
            ...
            <tfoot>
                <tr>
                    ...
                    <td style="text-align:right; font-weight:bold;">{{ number_format($ajustes->valor_subtotal, 2) }}</td>
                </tr>
                <tr>
                    <th colspan="6" style="text-align:right;">IVA</th>
                    <td style="text-align:right; font-weight:bold;">{{ number_format($ajustes->valor_iva, 2) }}</td>
                </tr>
                <tr>
                    <th colspan="6" style="text-align:right;">Total</th>
                    <td style="text-align:right; font-weight:bold;">{{ number_format($ajustes->valor_total, 2) }}</td>
                </tr>
            </tfoot>
        </table>
        <!-- PIE DE PÁGINA -->
    ...
    <script type="text/php">
        if (isset($pdf)) {
            ...
            $pdf->page_text(30, 565, "Ajuste de Inventario", $font, $size, $color);
        }
    </script>
</body>
</html>
